...
if __name__ == "__main__":
    ...
    lb_time = 690000 # left time bound
    rb_time = 1200000 # right time bound
    time_interval = 10000
    ...
    for i, evs_file in enumerate(evs_files):
        if i == 0:
            lb_time = 1094200 # left time bound
            rb_time = 1502200 # right time bound
        elif i == 1:
            lb_time = 1008600 # left time bound
            rb_time = 1364600 # right time bound
        elif i == 2:
            lb_time = 810800 # left time bound
            rb_time = 1552200 # right time bound
        # load .raw files
        ...
        for j, patch in enumerate(mask_patches[:]):
            ...
            if len(mask_evs) <= mask_len**2:
                ...
                # num_ev_mean is number of average events in patch at each time interval
                num_ev_mean = plot_num_ev_mean(mask_evs, mask, lb_time, rb_time, time_interval)    
                diff = np.diff(np.array(num_ev_mean))
                # diff is quantity of event increase between time intervals
                diff = diff / diff.max()
                ...
                # timestamp_map is event slope factor at each pixel (hight × width)
        ...
        # np.where((timestamp_map_mask != 0) & (timestamp_map_mask != max_interval)) の例外処理
